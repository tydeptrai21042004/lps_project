...
def _make_identity_half(half_len: int, device=None, dtype=None) -> torch.Tensor:
    half = torch.zeros(half_len, device=device, dtype=dtype)
    half[-1] = 1.0
    return half
...
class SymmetricConv1d(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.causal:
            ...
            y = F.conv1d(x, w, bias=self.bias, stride=1, padding=0, dilation=self.dilation, groups=self.groups)
        else:
            pad = self.dilation * (self.kernel_size - 1) // 2
            y = F.conv1d(x, w, bias=self.bias, stride=1, padding=pad, dilation=self.dilation, groups=self.groups)
        return self.h * y
...
class FixedSmoother1d(nn.Module):
    ...
    def __init__(
        ...
        if smoother_type == "gaussian":
            base = _make_gaussian_full(kernel_size)
        elif smoother_type == "hamming":
            base = _make_hamming_full(kernel_size)
        elif smoother_type == "savgol":
            base = _make_savgol_full(kernel_size, polyorder=polyorder)
        elif smoother_type == "moving_avg":
            base = torch.ones(kernel_size) / kernel_size
        else:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if self.causal:
            ...
            return F.conv1d(x, self.weight, groups=self.channels)
        pad = (self.kernel_size - 1) // 2
        return F.conv1d(x, self.weight, padding=pad, groups=self.channels)
